...
public class Batalla {
    ...
    public void iniciar() {
        ...
        Guerrero j1 = new Guerrero("Paladin", 80, 40); j1.inventario.add(i1); j1.inventario.add(i6); //se instancias los posibles jugadores
        ...
        Explorador j6 = new Explorador("Cazador", 30, 26); j6.inventario.add(i1); j6.inventario.add(i7); j6.inventario.add(i3);
        ...
        EnemigoNormal e1 = new EnemigoNormal("Bestia", 70, 22); e1.habilidades.add(h1);//se instancian los posibles enemigos
        ...
        EnemigoJefe e6 = new EnemigoJefe("Hidra", 60, 36); e6.habilidades.add(h1); e6.habilidades.add(h6);
    ...
    public void jugar() {
        int cantEnemigosN = rand.nextInt(3) + 1; //número entre 1 y 3
        int cantEnemigosJ = rand.nextInt(3) + 1; //número entre 1 y 3
        int cantJugadoresG = rand.nextInt(3) + 1;
        int cantJugadoresE = rand.nextInt(3) + 1;
        int cantEnemigos = cantEnemigosJ + cantEnemigosN; //cantidad total de enemigos
        int cantJugadores = cantJugadoresE + cantJugadoresG;
        vista.bienvenida(cantEnemigos, cantJugadores);
        for (int i = 0; i < cantJugadoresE; i++) { //seleccion de jugadores
            vista.menuSeleccionE(jugadoresE);
            int e = vista.pedirNumero("Selecciona tu explorador: ");
            jugadores.add(jugadoresE.get(e-1));
            jugadoresE.remove(e-1);
        }
        ...
        for (int i = 0; i < cantEnemigosN; i++) { //selección al azar d eos enemigos
            int aleatorio = rand.nextInt(enemigosN.size());
            enemigos.add(enemigosN.get(aleatorio));
            enemigosN.remove(aleatorio);
        }
        for (int i = 0; i < cantEnemigosJ; i++) { //se asegura de que haya tanto jefes como normales
            int aleatorio = rand.nextInt(enemigosJ.size());
            enemigos.add(enemigosJ.get(aleatorio));
            enemigosJ.remove(aleatorio);
        }

        vista.mostrarCombatientes(jugadores, enemigos);
        int salir = 0;
        while (salir == 0) {
            ...
            switch (opcion){
                case "ATACAR":
                    if (turno % 2 == 0) { //turno de los jugadores
                        vista.mostrarMensaje(jugadorTurno.mostrarMensajeInicio());
                        int objetivo_A_J_I = rand.nextInt(enemigos.size()); //objetivo ataque del jugador (índice)
                        Enemigo objetivo_A_J = enemigos.get(objetivo_A_J_I);
                ...
                case "PASAR EL TURNO":
                    ...
                    break;
                case "USAR ITEM":
                    int objetivo_I_J_I = rand.nextInt(enemigos.size()); //escoge un enemigo aleatorio para recibir el item
                    Enemigo objetivo_I_J = enemigos.get(objetivo_I_J_I);
                    vista.mostrarItems(jugadorTurno);
                    int item = vista.pedirNumero("Selecciona el número del item que quieras usar: ");
                    jugadorTurno.usarItem(item-1, objetivo_I_J, jugadorTurno, enemigos, jugadores, objetivo_I_J_I, turnoJ, cantEnemigos, cantJugadores);
                    if (objetivo_I_J.vivo()) {
                ...
                case "USAR HABILIDAD":
                    ...
                    Jugador objetivo_H_E = jugadores.get(objetivo_H_E_I);
                    vista.mostrarHabilidades(enemigoTurno);
                    int habilidad = vista.pedirNumero("Selecciona el número de la habilidad que quieras usar: ");
                    enemigoTurno.usarHabilidad(habilidad-1, enemigoTurno, objetivo_H_E, enemigos, jugadores, turnoE, objetivo_H_E_I, cantEnemigos, cantJugadores);
                    if (objetivo_H_E.vivo()) {
